...
>
> **Last updated**: 2026-05-08 — **JH-0 through JH-4 RESOLVED, JH-5 Phase 2 (local complete), JH-2 RESOLVED, GAP-11 CLOSED (18/18), 389 methods. All 6 projectNUCLEUS gaps addressed.**
...
| **toadStool** | S199-232: **S232: JH-2 RESOLVED** — full envelope enforcement (`mem_mb`, `cpu_cores`, `max_timeout_ms`) on all dispatch paths (`compute.dispatch.submit`, `shader.dispatch`, `pipeline.submit`). Pipeline stages propagate `CallerContext`. `-32004 RESOURCE_EXHAUSTED`. **S229-230: JH-0** adopted. 21,600+ tests | JH-0, JH-2, PG-05, pipeline scheduling | Pipeline envelope test coverage (logic threaded, test thinner) |
...
| **biomeOS** | v3.01-3.48: **v3.48: JH-2 RESOLVED** — `timeout_ms` enforced on forwarding path, `_resource_envelope` forwarded to downstream primals, 7,919 tests. **v3.47: JH-3 RESOLVED** (`composition.reload`). **v3.46: JH-0** MethodGate (13/13). `capability.resolve`, `inference.*` routing. | JH-0, JH-2, JH-3, capability.resolve, composition.reload | mem/cpu enforcement delegated to downstream (ToadStool enforces) |
...
| NG-13 | `unwrap()` in rpc/discovery — service reliability risk (Phase 60 audit) | P3 | **FALSE POSITIVE** (May 8, S59) — exhaustive audit: **zero `unwrap()` in production code** across `nestgate-rpc/src/rpc/` and `nestgate-discovery/src/`. All unwrap calls (hundreds) are confined to `#[cfg(test)]` blocks. Production paths use `?` propagation, `ok_or_else()`, `map_err()`, and `NestGateError` typed returns. |
| NG-14 | `storage.retrieve` for large/streaming tensors (Phase 60 carry-forward) | Medium | **FALSE POSITIVE** — implemented since Session 43p. `storage.store_stream`, `store_stream_chunk`, `retrieve_stream`, `retrieve_stream_chunk`, `object.size` all wired in dispatch table and registered in capabilities. |
| NG-15 | Cross-spring persistent storage IPC (Phase 60 carry-forward) | Medium | **FALSE POSITIVE** — implemented since Session 55+. `family_id` parameter on all storage calls enables cross-family scoping without biomeOS intermediary. |

**Compliance** (Session 59 — May 8): Clippy **CLEAN** (pedantic+nursery, 0 warnings), fmt **PASS**, **8,915 lib tests PASS** ↑. `forbid(unsafe_code)` per-crate + workspace `deny`. `deny.toml` present. SPDX present. Zero files >800L in production. Zero mocks in production. Zero `unwrap()` in production code. **66 UDS methods** (63→66 via JH-0 auth.*). **JH-0 MethodGate ADOPTED** — `method_gate.rs` pre-dispatch gate, Public/Protected classification, `NESTGATE_AUTH_MODE` enforcement, `auth.check`/`auth.mode`/`auth.peer_info` introspection. **BTSP Phase 1 COMPLETE**. **BTSP Phase 2 COMPLETE**. **BTSP Phase 3 COMPLETE**. **Capability Wire Standard L3**.
...
| **JH-2** | Token-carried resource envelope. **RESOLVED** (May 8): biomeOS v3.48 enforces `timeout_ms` on forwarding path (`min(system, cap)`), forwards `_resource_envelope` to downstream. ToadStool S232 enforces `mem_mb`, `cpu_cores`, `max_timeout_ms` on all dispatch paths (`compute.dispatch.submit`, `shader.dispatch`, `pipeline.submit`) with `-32004 RESOURCE_EXHAUSTED`. Pipeline stages propagate `CallerContext`. 7,919 + 21,600+ tests. | biomeOS + ToadStool | High | **RESOLVED** |
...
| **JH-5** | Log aggregation + provenance pipeline. **PHASE 2 COMPLETE** (skunkBat, May 8): All 7 event kinds now emitted from live code paths — `ThreatDetected` (security.detect), `DefenseAction` (security.respond), `BtspNegotiate` (try_negotiate_upgrade), `BtspDecryptFailure` (encrypted frame loop), `LifecycleTransition` (start/stop), plus gate events. `capabilities.list` L3 drift fixed (`audit_log` in security methods). 363 tests. **Remaining**: rhizoCrypt DAG + sweetGrass braid forwarding (cross-primal IPC, deferred until ionic token auth is standard). | skunkBat team | Medium | **PHASE 2** — local complete, cross-primal forwarding deferred |
...
~~rhizoCrypt: DID vs raw public_key semantic gap~~ **RESOLVED** (S63) — `signer_did`/`attester_did` type rename with `#[serde(rename = "public_key")]` wire compat. Coverage: **93.88%**. Tests: **1,507**.
...
| **JH-2: Token-carried resource envelope** — biomeOS v3.48 + ToadStool S232: full enforcement (mem, cpu, timeout) on all dispatch paths. `_resource_envelope` forwarded. `-32004 RESOURCE_EXHAUSTED`. | projectNUCLEUS (JupyterHub) | **biomeOS + ToadStool** | **RESOLVED** (May 8) |
...
| **JH-5: Log aggregation + provenance** — skunkBat Phase 2: all 7 event kinds instrumented, `capabilities.list` L3 fixed. Local audit complete. Cross-primal forwarding (rhizoCrypt/sweetGrass) deferred. | projectNUCLEUS (JupyterHub) | **skunkBat team** | **PHASE 2** (May 8) — local complete |
