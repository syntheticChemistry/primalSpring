...
### ~~Remaining Upstream BTSP Debt~~ — ALL RESOLVED (April 24, 2026)

All 13/13 capabilities BTSP-authenticated. Final convergence fixes:
- **Songbird** (Wave 169): `SecurityRpcClient::new_direct()` for direct BearDog calls
- **ToadStool**: post-handshake connection persistence for NDJSON RPC
- **loamSpine**: `btsp.negotiate` non-fatal fallback (BearDog doesn't implement this method)
- **petalTongue**: BearDog field alignment, full BTSP server converged
...
- **MitoSeed**: deterministic, clonable seed from `FAMILY_SEED` or `.family.seed` file.
  Proves group membership. Used for BTSP Phase 1 handshake key derivation.
- **Public seed fingerprints**: BLAKE3 hash of each primal's canonical crypto parameters
  published in `plasmidBin/manifest.toml`. Layer 0.5 verifies binary authenticity before
  any IPC occurs — untampered primals safe for use.
...
- Updated `PRIMALSPRING_COMPOSITION_GUIDANCE.md` (Section 19), `UPSTREAM_CROSSTALK_AND_DOWNSTREAM_ABSORPTION.md` (convergent evolution model), `PLASMIDBIN_DEPOT_PATTERN.md` (interactive composition path)
