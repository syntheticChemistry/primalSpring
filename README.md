...
| **Mesh** | 8-node sovereign relay (golgi ↔ sporeGate ↔ eastGate ↔ flockGate ↔ ironGate ↔ northGate ↔ southGate + darwinGate). WG overlay 10.13.37.0/24. TOML-driven topology (`config/mesh_topology.toml`). K-Derm cytoplasm zones: backbone/house2/garage/wan. Three-hub triangle topology. Live overlay validation (s_mesh_overlay). `gate.enroll` automated enrollment pipeline. Sovereignty roadmap validation (Tower Atomic → WG replacement). |
