...
**Version**: 0.9.17+ (Phase 46 — 631 tests, 75 experiments, 67 deploy graphs, guideStone Level 4 — **187/187 live NUCLEUS ALL PASS (13/13 BTSP authenticated, 8 cellular graphs BTSP-enforced)** + 41/41 bare, BLAKE3 checksums (P3), seed provenance (Layer 0.5), **BTSP default on all tiers** (Layer 1.5), cellular deployment (Layer 7), BTSP Phase 1–3, multi-tier genetics (Mito-Beacon / Nuclear / Tags), capability-based discovery (zero hardcoded primal names), **genomeBin v5.1 — 46 binaries across 6 target triples (Tier 1: 39/39)**, shell composition library (41 functions), composition template + reference implementation, per-spring exploration lanes)  
