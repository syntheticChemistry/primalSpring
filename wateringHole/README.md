...
| BTSP | **Phase 1–3 (CONVERGED)**: FAMILY_SEED auth → secure-by-default cascade → ChaCha20-Poly1305 encrypted channel. **BTSP default on all tiers**: guidestone expects BTSP on Tower, Node, Nest, Provenance. **13/13 BTSP authenticated (171/171 checks ALL PASS)**. JSON-line BTSP auto-detect wired in all relay primals. 15s relay handshake timeout. Two-pass `upgrade_btsp_clients()` |
...
| NUCLEUS | **VALIDATED** — Tower + Nest + Node, 12/12 primals ALIVE, **171/171 guidestone ALL PASS (13/13 BTSP)**, 19/19 exp094 parity, 12/12 exp091 routing |
...
## Track Structure (17 Tracks, 75 Experiments)
