...
- `LaunchError` — typed: BinaryNotFound, SpawnFailed, SocketTimeout, ProfileParseError

### `harness/` Module
- `AtomicHarness::start(AtomicType, family_id)` — spawns a full composition (e.g. Tower = beardog + songbird)
- `RunningAtomic` — RAII lifecycle, health_check_all(), capabilities_all(), validate(), socket_for_primal(), client_for()
...
- All `#[ignore]` guarded — requires `ECOPRIMALS_PLASMID_BIN` and live binaries
...
|--------|--------|-------|
| Unit tests | 225 | 233 |
| Integration tests | 10 | 13 (10 + 3 live) |
| Doc-tests | 1 | 2 |
| Total | 236 | 248 |
