...
//!   1. Scope pattern matching (pure logic, no IPC)
//!   2. MethodGate enforcement with verified tokens (pure logic)
//!   3. Bearer token extraction from JSON-RPC params
//!   4. Live token issuance via BearDog (`auth.issue_ionic`)
//!   5. Authenticated composition call with bearer threading
//!
//! Phases 1-3 always run. Phases 4-5 require live NUCLEUS.
...
fn main() {
    ValidationResult::new("primalSpring Exp108 — Ionic Token Federation")
        .with_provenance("exp108_token_federation", "2026-05-09")
        .run(
            "Exp108: Token Federation — JH-11 scope enforcement pipeline",
            |v| {
                ...
                phase_live_token_issuance(v);

                v.section("Phase 5: Authenticated Composition");
                phase_authenticated_composition(v);
            },
        );
}
...
fn phase_live_token_issuance(v: &mut ValidationResult) {
...
fn phase_authenticated_composition(v: &mut ValidationResult) {
    let mut ctx = CompositionContext::from_live_discovery_with_fallback();
