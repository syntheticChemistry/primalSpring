    ...
    ValidationResult::run_experiment(
        ...
        |v| {
            ...
            let running = match AtomicHarness::new(AtomicType::Node).start(&family_id) {
                ...
                Err(e) => { v.check_bool("node_startup", false, &format!("{e}")); return; }
            };
            ...
            if let Some(ts) = running.socket_for("compute").or_else(|| running.socket_for_primal("toadstool")) {
                ...
                v.check_bool(
                    "toadstool_caps",
                    caps.as_ref().is_ok_and(|v| v["supported_workload_types"].is_array()),
                ...
                if let Ok(c) = &caps {
                    ...
                    let cores = u64_to_usize(c["available_resources"]["total_cpu_cores"].as_u64().unwrap_or(0));
