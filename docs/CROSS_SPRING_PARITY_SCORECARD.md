...
**Last updated**: May 24, 2026 (Wave 47 — sporePrint Rust evolution, gate deployment intel absorbed, version sweep). biomeOS v3.70, 458 methods, 784 primalSpring tests. NeuralBridge feedback loop operational. All 13 primals at zero debt.
...
|--------|------:|-------------------|-----------------|-----------------|-------------------|--------------|----------------------|----------------|-----------|
| **primalSpring** | 784 | None (validates, doesn't consume) | N/A (is primalSpring) | L8 (absorbed) | **G** 458 methods, sync-tested | **G** 80 deploy + 14 signal graphs | **G** 89 exp crates + 49 scenarios (10 tracks) + Waves 1-47 complete, feedback loop + observatory posture + live validation | **Y** 5 (frozen JSON, not live paper) | **G** bans ring/openssl |
...
| **neuralSpring** | 739 | **G** optional=true, IPC-first default (`default=[]`) | **Y** feature-gated | **G** L5 (10 scenarios incl. s_schema_standard + s_nest_commit) | **G** 37 caps + sync 458 + primal.list, stability tiers annotated | **G** 4 graphs (V165 + execute_graph_live) | **G** exp094 parity + s_signal_dispatch + store_science_result + node.compute dispatch, degradation doc | **G** 10 (paper-linked, DOI) | **G** bans ring/openssl/rustls |
| **ludoSpring** | 982 | **G** optional=true, IPC-first default (`default=["ipc"]`) | **Y** feature-gated | **G** L4 (Tier 1-3, 3-tier certification) | **G** canonical flat array + count + cross-sync 458, stability tiers | **G** 12 graphs, compute trio in cells (V75) | **G** 5 composition scenarios + Bartle + gamification + Schell Lenses + CPU/GPU parity | **R** 0 .ipynb (Python baselines in baselines/) | **G** bans ring/openssl |
| **groundSpring** | 1,123+ | **G** optional=true, IPC-first default (`default=[]`) | **Y** feature-gated | **G** L4 (modularized 5-layer guidestone) | **G** 20 IPC methods + registry sync 458 + canonical envelope | **G** 6 graphs | **G** LTEE B1-B4+B6 + nest.commit dispatch + signal dispatch, degradation doc | **G** 34+ (paper-linked, LTEE B6 BioBrick) | **G** bans ring/openssl |
| **airSpring** | 1,373 | **G** optional=true, IPC-first default (`default=[]`) | **Y** feature-gated (guidestone) | **G** L5 (Wave 20, 10 scenarios) | **G** 57 caps (51→57) + cross-sync 458 + canonical envelope + --provenance-dir, stability tiers (53/4) | **G** 7 graphs | **G** 3 exp crates + 3 new cross-tier parity validators (autocorrelation, gamma_cdf, soil_moisture_topp) + 17 full parity methods | **G** 25 (paper-linked) | **G** bans ring/openssl/aws-lc-sys |
...
- **Sync-tested** (highest maturity): primalSpring (canonical 458), hotSpring, neuralSpring (34 caps), groundSpring (16 MCP + 6 sync tests), healthSpring (118 methods + CI cross-sync), ludoSpring (28 game.* + cross-sync 458), wetSpring (cross-sync 458), airSpring (46 caps + cross-sync 458)
- All 8 springs CI-validated against canonical 458
...
**Metrics**: 49 scenarios (incl. zero-port standard), 77 deploy graphs, 322/458 methods exercised (70% coverage,
up from 69%). Remaining 136 uncovered are test fixtures, domain-specific (game/nautilus/ml),
...
| Metric | Value |
|--------|-------|
| Total tests across 8 springs | **9,579+** (primalSpring 784 + hotSpring 596 + healthSpring 1,018 + wetSpring 1,962 + neuralSpring 739 + ludoSpring 982 + groundSpring 1,123 + airSpring 1,373 + metalForge/integration/Python suites) |
...
| Registered capability methods | **458** (primalSpring canonical, post-recount, test fixtures excluded) |
