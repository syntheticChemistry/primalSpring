...
- Capability registry string matching against canonical 460 methods
...
- `build_registry()` returns exactly 56 scenarios
...
|------|------:|----------------------|
| Rust | 11 | Assert `v.failed == 0` (full structural pass) |
| Both | 11 | Test structural phase or verify no panics |
| Live | 27 | Verify scenario runs to completion (failures expected without primals) |

`cargo test --lib` exercises all 56 scenarios — the single authoritative CI gate.
