...
| toadStool | 23,000+ | **ADOPTED** | FULL | L3 | **CLEAN** — S324+: 112 methods (17 groups), zero-copy dispatch, riboCipher CLEAR+MITO, gRPC/OpenCL deleted (S319), 100% SPDX, zero files >750L, zero hardcoded cross-primal names, `socket_env` SSOT (zero raw env literals), workspace deps unified, reagent module split, Duration dedup, client riboCipher fixed (12 tests unblocked S322), composition graduated (+11 S322), ipc_watch covered (9 tests S322), test extraction complete (method_gate/job/shader_dispatch S323), submit param split (S323), cpu_resource resilience extraction (S323), flaky test fix (S323), edge communication coverage (+9 tests S323), test graduation (6 un-ignored S324), catalyst watchdog coverage (15 tests S324), dispatcher mock E2E (7 tests S324), MMIO split (613→275+179+194L S324). 9,095+ lib tests. |
...
**ALL PRIMAL BLOCKERS RESOLVED.** grapheneGate 13/13 structurally ready.

| Gap | Owner | Severity | Description |
|-----|-------|----------|-------------|
| ~~**CR-TARPC-01**: coralReef tarpc fatal on tcp_only~~ | **coralReef** | ~~**P2**~~ | **RESOLVED** (b1ec1f4). tarpc skips `bind()` on `tcp_only`. grapheneGate → 12/13. |
