...
| toadStool | 23,000+ | **ADOPTED** | FULL | L3 | **CLEAN** — S322+: 112 methods (17 groups), zero-copy dispatch, riboCipher CLEAR+MITO, gRPC/OpenCL deleted (S319), 100% SPDX, zero files >750L, zero hardcoded cross-primal names, `socket_env` SSOT (zero raw env literals), workspace deps unified, reagent module split, Duration dedup, client riboCipher fixed (12 tests unblocked), composition graduated (+11 tests), ipc_watch covered (9 tests), telemetry wire-contract constants. 9,065+ lib tests. |
