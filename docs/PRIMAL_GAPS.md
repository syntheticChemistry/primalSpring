...
>
> **Last updated**: 2026-06-10 (Wave 107 ecosystem: **ZERO P1.** grapheneGate **11/13 LIVE** on Pixel 8a via TCP-only. **2 blocked**: coralReef (CR-TARPC-01: tarpc UDS bind fatal), biomeOS (BM-UDS-01: Neural API UDS bind fatal). `PRIMAL_BIND_MODE=tcp_only` adopted 10/13, workaround 1/13 (nestGate), blocked 2/13. **Zero-port standard CLEAN**: droppable federation ports eliminated (beardog 9900/9101, skunkBat 9750 all DROPPED). toadStool socket cleanup VERIFIED (3-tier resolution adopted). 4-gate mesh + grapheneGate = 5-gate ecosystem. 901 tests, 55 scenarios. deploy_pixel.sh hardened for TCP-only substrate. grapheneGate scenario tracks upstream blockers — will auto-pass when fixes land.)
>
...
**270+ PASS, 0 FAIL, 3 KNOWN_DEBT (upstream blockers)** — projectNUCLEUS Phase 60+ validation, darkforest v0.2.1. primalSpring: **55 scenarios** (10 tracks, 3 tiers), 490+ real methods (100% exercised), **901 lib tests pass** (2 ignored) + **17 doc tests**, zero clippy warnings, zero `#[allow]` in production, 3 binaries (UniBin + IPC server + nucleus_launcher). **16 runtime deps**. Pure Rust crypto for BTSP bootstrap. **Wave 107: ZERO P1. 3 KNOWN_DEBT (upstream blockers tracking CR-TARPC-01, BM-UDS-01 in grapheneGate scenario — will auto-clear when fixes land).** grapheneGate **11/13 LIVE** on Pixel 8a (bearDog, songBird, skunkBat, toadStool, barraCuda, nestGate, rhizoCrypt, loamSpine, sweetGrass, squirrel, petalTongue). `PRIMAL_BIND_MODE=tcp_only` validated on real Android SELinux substrate. 2 blocked: coralReef (tarpc UDS fatal on `bind()`), biomeOS (Neural API UDS fatal on `bind()`). **Zero-port standard CLEAN**: droppable federation ports eliminated (beardog 9900/9101, skunkBat 9750 all DROPPED — port 9900 reassigned to petalTongue TCP). songBird M1 SHIPPED (ff86204c): `ipc.resolve` topology-aware mesh routing. barraCuda `method.describe` SHIPPED (v0.4.0): 97 methods, runtime introspection. **Socket cleanup 4/5 DONE**: sweetGrass v0.7.55, barraCuda v0.4.0, coralReef Wave 107, squirrel Wave 107 — all zero `/tmp` artifacts. **toadStool 3-tier resolution VERIFIED** (BIOMEOS_SOCKET_DIR + XDG_RUNTIME_DIR adopted; `/tmp` fallback via `temp_dir()` only when no env set). **NUCLEUS supervision SHIPPED** (biomeOS v4.17). **5-gate ecosystem** (eastGate↔golgiBody↔ironGate+southGate + grapheneGate standalone). southGate cross-subnet VALIDATED (13/13). gate.bootstrap **SHIPPED + HARDENED**. Transport 11/11 COMPLETE. Depot 13/13 BLAKE3 VERIFIED. WAN depot 13/13 serving. S1-S4 GRADUATED. **Remaining P2**: coralReef CR-TARPC-01, biomeOS BM-UDS-01, flockGate WAN e2e 5/5.
...
**11/13 CLEAN, 2/13 P2 — Waves 1-107. 55 scenarios (10 tracks), 490+ methods (100% exercised), 901 lib tests, 3 known debt (upstream blockers). Wave 107: ZERO P1. grapheneGate 11/13 LIVE on Pixel 8a. coralReef P2 (CR-TARPC-01: tarpc fatal on tcp_only), biomeOS P2 (BM-UDS-01: Neural API UDS fatal on tcp_only). Federation ports CLEAN (beardog 9900/9101 + skunkBat 9750 DROPPED). toadStool 3-tier socket resolution VERIFIED. songBird topology-aware routing (ff86204c). barraCuda method.describe (v0.4.0). Socket cleanup 4/5 DONE (toadStool verified but temp_dir fallback remains without env). 5-gate ecosystem. S1-S4 GRADUATED. Remaining P2: coralReef tarpc gate, biomeOS UDS gate, flockGate WAN.**
...
| **toadStool** | `/tmp/biomeos/compute-tarpc.sock`, `/tmp/biomeos/compute.sock`, `/tmp/toadstool-jsonrpc-port` | `/run/membrane/toadstool.sock` only — **PARTIALLY RESOLVED** (3-tier: `BIOMEOS_SOCKET_DIR` → `XDG_RUNTIME_DIR` → `temp_dir()` fallback. No hardcoded `/tmp` literals in prod. Remaining: `temp_dir()` fallback still lands in `/tmp` when no env set.) |
