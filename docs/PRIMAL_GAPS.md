...
>
> **Last updated**: 2026-06-10 (Wave 106: grapheneGate **9/13 primals RUNNING** on Pixel 8 — TCP-only fallback infrastructure **SHIPPED** (`ipc::server_bind` module: `bind_transport()` + `BindMode::Fallback` + `PRIMAL_BIND_MODE` env). 4 remaining primals (coralreef, nestgate, biomeOS, petaltongue) can now adopt `bind_transport(slug, BindMode::from_env())` for graceful UDS→TCP degradation. Scenario `s_tcp_fallback` (62nd) validates fallback chain. primalSpring own server binary uses `bind_transport` as reference implementation. `deploy_pixel.sh` exports `PRIMAL_BIND_MODE=fallback`. 3-gate mesh collective LIVE (eastGate↔golgiBody↔ironGate). 897 tests 0 failures.)
>
