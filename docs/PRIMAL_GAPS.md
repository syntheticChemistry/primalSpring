...
| **BearDog** | Wave 34-93: **Wave 93: JH-0 MethodGate** — pre-dispatch auth on UDS+TCP, `BEARDOG_AUTH_MODE`, `-32001/-32000/-32002` error codes, `auth.check`/`auth.mode`/`auth.peer_info`, 114 methods. Wave 91: typed errors. Wave 89: `crypto.did_from_key`. Ionic bond lifecycle, BTSP server live. | BTSP server, ionic bond, crypto.did_from_key, JH-0 MethodGate | JH-1 (identity.create, auth.issue_ionic) not yet shipped; bond persistence NestGate wiring |
...
|-----|-------|-------|----------|--------|
| **JH-0** | RPC dispatcher capability check. **7/13 primals adopted** (May 8): BearDog W93 (UDS+TCP, `-32001`, `BEARDOG_AUTH_MODE`), ToadStool S229 (`-32006` — **diverges from standard**, needs alignment), barraCuda S54 (`-32001`, `BARRACUDA_AUTH_MODE`), biomeOS v3.46 (`-32001`, `_bearer_token` extraction — best wiring), coralReef I94 (`-32001` defined but **wire bug**: gate denial emits `-32000` via `IpcPhase::Handler` mapping), skunkBat JH-0 (`-32001`, `SKUNKBAT_AUTH_MODE`), primalSpring v0.9.25 (reference). **Not yet adopted**: NestGate, Songbird, rhizoCrypt, loamSpine, sweetGrass, petalTongue. All adopters ship `auth.check`/`auth.mode`/`auth.peer_info` + permissive default. No primal validates tokens yet (presence-only check). | All primal teams + primalSpring (pattern) | **Critical** | **IN PROGRESS** (7/13) |
...
| **JH-0: RPC dispatcher capability check** — **7/13 adopted** (May 8): primalSpring (ref impl), BearDog W93, ToadStool S229, barraCuda S54, biomeOS v3.46, coralReef I94, skunkBat. All ship `auth.*` trio + permissive default. **Divergences**: ToadStool uses `-32006` (should be `-32001`); coralReef wire bug emits `-32000` for gate denial. **Not adopted**: NestGate, Songbird, rhizoCrypt, loamSpine, sweetGrass, petalTongue. No primal validates token scopes yet. | projectNUCLEUS multi-user pentest | **All primal teams + primalSpring** | **IN PROGRESS** (7/13) — Critical. Next: remaining 6 primals adopt; ToadStool/coralReef fix error code alignment |
