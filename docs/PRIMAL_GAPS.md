...
| toadStool | 23,000+ | **ADOPTED** | FULL | L3 | **CLEAN** — S325+: 112 methods (17 groups), zero-copy dispatch, riboCipher CLEAR+MITO, gRPC/OpenCL deleted (S319), 100% SPDX, zero files >750L, zero hardcoded cross-primal names, `socket_env` SSOT (zero raw env literals), workspace deps unified, reagent module split, Duration dedup, kernel sentinel coverage (28 tests S325), biomeos path consolidation (3 sites → `get_biomeos_dir()` S325), hot-path clone elimination (submit_params + dispatcher S325), discovery fallback production gate (S325). 9,127+ lib tests. |
