...
| **biomeOS** | v3.01-3.47: **v3.47: JH-2** (ResourceEnvelope parsing — mem, cpu, method_allowlist — allowlist enforced at gate), **JH-3 RESOLVED** (`composition.reload` wired — apoptosis → drain → re-register → health check). **v3.46: JH-0 MethodGate** (13/13 adopted, bearer token extraction). `capability.resolve`, `lifecycle.composition`, `inference.*` routing. 7,911 tests | JH-2 (partial), JH-3, JH-0, capability.resolve, composition.reload | `cpu`/`timeout` envelope enforcement, `_bearer_token` stripping on non-semantic paths |
