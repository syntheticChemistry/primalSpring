...
>
> **Last updated**: 2026-06-10 (Wave 106: grapheneGate **9/13 primals RUNNING** on Pixel 8 — TCP-only fallback infrastructure **SHIPPED** (`ipc::server_bind` module: `bind_transport()` + `BindMode::Fallback` + `PRIMAL_BIND_MODE` env). 3 remaining primals (coralreef, nestgate, petaltongue) can adopt `bind_transport(slug, BindMode::from_env())` for graceful UDS→TCP degradation. biomeOS adopted (v4.18) → grapheneGate 10/13. Scenario `s_tcp_fallback` (62nd) validates fallback chain. primalSpring own server binary uses `bind_transport` as reference implementation. `deploy_pixel.sh` exports `PRIMAL_BIND_MODE=fallback`. NUCLEUS supervision SHIPPED (biomeOS v4.17 — LifecycleManager auto-restarts). 3-gate mesh collective LIVE (eastGate↔golgiBody↔ironGate). ZERO P1 remaining. 897 tests 0 failures.)
>
...
**267+ PASS, 0 FAIL, 1 KNOWN_DEBT** — projectNUCLEUS Phase 60+ validation, darkforest v0.2.1. primalSpring: **62 scenarios** (10 tracks, 3 tiers), 490+ real methods (100% exercised), **897 lib tests pass** (2 ignored) + **17 doc tests**, **5 clippy warnings** (borderline function-length only), zero `#[allow]` in production, 3 binaries (UniBin + IPC server + nucleus_launcher). **16 runtime deps**. Pure Rust crypto for BTSP bootstrap. **Wave 106: ZERO P1 remaining. NUCLEUS supervision SHIPPED (biomeOS v4.17). biomeOS TCP-only fallback SHIPPED (v4.18). 3-gate mesh collective LIVE** (eastGate↔golgiBody↔ironGate). **Mesh persistence SHIPPED** (peers.toml + auto-reconnect). **Federation port 7700 LISTENING** (bound *, LAN+WAN). gate.bootstrap **SHIPPED** (strandGate re-enrollment VALIDATED). **TCP-only fallback infrastructure SHIPPED** (`ipc::server_bind` module — `bind_transport()`, `BindMode::Fallback`, `PRIMAL_BIND_MODE` env var). biomeOS v4.16 `graph.deploy` VALIDATED with `LocalTrusted`. bearDog v0.9.0 pure Rust. Transport 11/11 COMPLETE. Depot 14/14 BLAKE3 VERIFIED. WAN depot 13/13 serving. Cascade **38/38 clean**. aarch64-musl 13/13 BUILT, grapheneGate **9/13 RUNNING** on Pixel 8 (beardog, songbird, skunkbat, toadstool, barracuda, rhizocrypt, loamspine, sweetgrass, squirrel). **Known debt**: skunkBat TCP 9750 bound (zero-port standard violation, pending UDS-only migration). 3 primals (coralreef, nestgate, petaltongue) need to adopt `bind_transport(slug, BindMode::from_env())` for graceful UDS→TCP degradation. biomeOS already adopted (v4.18, grapheneGate 10/13). **Critical path**: ZERO P1 — autonomous gate operation unblocked. 3-gate collective is operational.
