...
use crate::composition::{self, CompositionContext};
use crate::coordination::AtomicType;
use crate::tolerances;
use crate::validation::ValidationResult;

/// Layer 2: `health.liveness` for every capability in Tower, Node, and Nest tiers.
...
/// Layer 3: stats/tensor IPC parity versus local baselines.
...
/// Layer 3: `storage.store` / `storage.retrieve` roundtrip on Nest.
...
/// Layer 3: shader.compile.capabilities response shape.
...
/// Layer 4: Tower hash → Nest store → retrieve (cross-atomic pipeline).
