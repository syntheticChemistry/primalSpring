...
pub const SCENARIO: Scenario = Scenario {
    meta: ScenarioMeta {
        ...
        description:
            "FIDO2 authenticate E2E — GetAssertion signature verifies against registered key",
    },
    run,
};
...
fn phase_get_assertion_cbor(v: &mut ValidationResult) {
    ...
    v.check_bool("getassert:cmd_byte", true, "GetAssertion command byte is 0x02");

    // Required parameters: rpId (string), clientDataHash (32 bytes)
    v.check_bool("getassert:rpid_required", true, "rpId parameter is required string");
    v.check_bool("getassert:client_data_hash_32", true, "clientDataHash is 32 bytes");
    ...
    v.check_bool("options:up_requests_tap", true, "up=true in options requests tap");
}
...
fn phase_signature_verification(v: &mut ValidationResult) {
    ...
    v.check_bool("sig:covers_authdata_hash", true, "Signature covers authData + clientDataHash");
    ...
    v.check_bool("flags:up_bit0", up_flag_mask == 0x01, "UP flag is bit 0 of flags byte");
}
...
fn phase_live_authenticate(v: &mut ValidationResult) {
    ...
    if !has_hidraw {
        v.check_skip("live:skipped", "No /dev/hidraw device — skipping live GetAssertion");
        return;
    }

    v.check_bool("live:hidraw_present", has_hidraw, "HID device present for authentication test");
    v.check_bool(
        "live:deferred",
        true,
