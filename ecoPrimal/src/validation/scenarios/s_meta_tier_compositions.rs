...
const META_COMPOSITION_PRIMALS: &[&str] = &["biomeos", "squirrel", "petaltongue", "skunkbat"];

fn validate_composition_tools_parse(v: &mut ValidationResult) -> Option<toml::Value> {
    match toml::from_str::<toml::Value>(COMPOSITION_TOOLS_TOML) {
        Ok(parsed) => {
            v.check_bool(
                "tools:parse",
                true,
                "composition_tools.toml parses as valid TOML",
            );
            Some(parsed)
        }
        Err(e) => {
            v.check_bool(
                "tools:parse",
                false,
                &format!("composition_tools.toml parse error: {e}"),
            );
            None
        }
    }
...
fn validate_tool_coverage(v: &mut ValidationResult, parsed: &toml::Value) {
    ...
    v.check_bool(
        "tools:has_entries",
        tool_count > 0,
        &format!("composition_tools.toml has {tool_count} tool entries"),
    );
    ...
    for &expected in EXPECTED_TOOL_NAMES {
        ...
        v.check_bool(
            ...
            present,
            &format!("composition_tools.toml includes tool definition for {expected}"),
        );
    }
...
fn validate_registry_meta_compositions(v: &mut ValidationResult) {
    ...
    for &primal in META_COMPOSITION_PRIMALS {
        let present = primals.contains(&primal);
        v.check_bool(
            &format!("registry:meta_primals:{primal}"),
            present,
            &format!("{primal} listed in compositions.meta.primals"),
        );
    }

    let composition_count = meta
        .get("compositions")
        .and_then(|s| s.as_array())
        .map_or(0, Vec::len);
    v.check_bool(
        "registry:meta_composition_count",
        composition_count == 5,
        &format!("compositions.meta declares {composition_count} compositions, expected 5"),
    );
}
...
pub fn run(v: &mut ValidationResult, _ctx: &mut CompositionContext) {
    ...
    if let Some(parsed) = validate_composition_tools_parse(v) {
    ...
    validate_registry_meta_compositions(v);
...
mod tests {
    ...
    #[test]
    fn meta_tier_compositions_pass() {
        let mut v = ValidationResult::new("meta-tier-compositions");
    ...
    #[test]
    fn composition_tools_parse() {
        let result: Result<toml::Value, _> = toml::from_str(COMPOSITION_TOOLS_TOML);
        assert!(
            result.is_ok(),
            "composition_tools.toml failed to parse: {:?}",
            result.err()
        );
    }
    ...
    fn all_tiers_covered_in_tool_schema() {
        let parsed: toml::Value = toml::from_str(COMPOSITION_TOOLS_TOML).unwrap();
