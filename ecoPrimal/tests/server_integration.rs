...
fn nest_storage_round_trip() {
    ...
    assert_eq!(retrieve.as_ref().unwrap()["data"]["message"], "hello from primalSpring");
}
...
fn node_toadstool_capabilities() {
    ...
    assert!(caps.is_ok(), "toadstool.query_capabilities should succeed: {caps:?}");
...
fn overlay_tower_ai_validation() {
    ...
    running.validate(&mut v);
    assert!(v.passed >= 2, "at least base tier checks should pass (got {})", v.passed);
...
fn overlay_graph_capability_map() {
    ...
    assert!(
        !caps.contains_key("coordination") || caps.get("coordination").unwrap() != "validate_tower_ai",
